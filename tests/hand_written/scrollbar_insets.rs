use taffy::prelude::*;
use taffy::{
    compute_block_layout, compute_flexbox_layout, compute_grid_layout, compute_leaf_layout_with_scrollbar_insets,
    compute_root_layout, Direction, LayoutBlockContainer, LayoutFlexboxContainer, LayoutGridContainer, LayoutInput,
    LayoutOutput, Point,
};
...
impl TestNode {
    fn container(display: Display, style: Style, scrollbar_insets: Rect<f32>) -> Self {
        Self {
    ...
    fn leaf(style: Style, measured_size: Size<f32>) -> Self {
        Self {
            style,
...
impl TestTree {
    fn new(root: TestNode, child: TestNode) -> Self {
        let mut nodes = vec![root, child];
        nodes[0].children.push(1);
        Self { nodes }
    }

    fn compute(&mut self, available_space: Size<AvailableSpace>) {
...
impl LayoutPartialTree for TestTree {
    ...
    fn compute_child_layout(&mut self, node_id: NodeId, inputs: LayoutInput) -> LayoutOutput {
        ...
        let scrollbar_insets = self.nodes[index].scrollbar_insets;
        if self.nodes[index].leaf {
            let measured_size = self.nodes[index].measured_size;
            return compute_leaf_layout_with_scrollbar_insets(
                inputs,
                &style,
                scrollbar_insets,
                |_value, _basis| 0.0,
