...
fn layout_grid_item_with_physical_margins(writing_mode: WritingMode, direction: Direction) -> Point<f32> {
    let mut tree = TaffyTree::<()>::new();
    let child = tree
        .new_leaf(Style {
            size: Size { width: length(40.0), height: length(20.0) },
            margin: Rect { top: length(4.0), right: length(8.0), bottom: length(12.0), left: length(16.0) },
            ..Style::default()
        })
        .unwrap();
    tree.set_writing_mode(child, writing_mode).unwrap();
    ...
    tree.compute_layout(container, Size::MAX_CONTENT).unwrap();
    tree.unrounded_layout(child).location
}
...
#[test]
fn physical_margins_align_from_logical_grid_start_edges() {
    for (writing_mode, direction, expected) in [
        (WritingMode::HorizontalTb, Direction::Ltr, Point { x: 16.0, y: 4.0 }),
        (WritingMode::VerticalRl, Direction::Ltr, Point { x: 32.0, y: 4.0 }),
        (WritingMode::VerticalLr, Direction::Ltr, Point { x: 16.0, y: 4.0 }),
        (WritingMode::HorizontalTb, Direction::Rtl, Point { x: 32.0, y: 4.0 }),
        (WritingMode::VerticalRl, Direction::Rtl, Point { x: 32.0, y: 48.0 }),
        (WritingMode::VerticalLr, Direction::Rtl, Point { x: 16.0, y: 48.0 }),
    ] {
        assert_eq!(
            layout_grid_item_with_physical_margins(writing_mode, direction),
            expected,
            "physical margins for {writing_mode:?} {direction:?}"
        );
    }
}
