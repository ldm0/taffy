...
use crate::{AutoSizeBehavior, BoxSizing, ResolvedAspectRatio};
...
pub(crate) struct IntrinsicAxisValue {
    ...
    /// Whether the value was synthesized from the preferred aspect ratio.
...
pub(crate) fn intrinsic_content_size_from_initial_geometry(
    ...
    IntrinsicAxisValue {
        value: resolved,
        applied_aspect_ratio: resolved.is_some(),
...
pub(crate) struct ContentBasedSizeConstraints {
    ...
    /// Whether the preferred size was synthesized from the preferred aspect ratio.
    pub preferred_aspect_ratio_applied: bool,
...
impl ContentBasedSizeConstraints {
    ...
    pub(crate) fn resolve_against(self, preferred: Option<f32>, constraints: ResolvedAxisConstraints) -> Self {
        ...
        Self {
            ...
            preferred_aspect_ratio_applied: preferred.is_none() && self.preferred_aspect_ratio_applied,
            min,
            max,
            automatic_min: None,
...
impl BlockSizeProperties {
    ...
    fn resolve_content_based_constraints(
        ...
        let preferred_aspect_ratio_applied =
            ratio_block_size.is_some() && (self.preferred.is_intrinsic() || resolves_auto_size_from_ratio);
        ...
        ContentBasedSizeConstraints {
            preferred: resolve_explicit(self.preferred)
                .or_else(|| (self.preferred.is_auto() && auto_size_is_content_based).then_some(content_block_size)),
            preferred_aspect_ratio_applied,
...
pub(crate) struct ContentBasedBlockSize {
    ...
    auto_size_is_content_based: bool,
    /// Floor applied to the formatting context's real intrinsic block size.
...
impl ContentBasedBlockSize {
    ...
    /// while leaving the final automatic size content-based.
    #[inline(always)]
    pub(crate) const fn has_available_intrinsic_floor(self) -> bool {
    ...
    /// Establish initial block geometry for descendant percentage resolution.
    ///
    /// An available-space intrinsic floor is not a parent-owned fixed size,
    /// but descendants still need the size produced by the normal
    /// preferred-ratio and min/max order before the formatting context lays
    /// out its content. Reusing this resolver keeps that initial geometry in
    /// lockstep with final content-based block-size resolution.
    pub(crate) fn apply_initial_block_geometry(
        ...
        let initial_constraints = self
            ...
            .resolve_against(logical_outer_size.block_size, self.resolved_constraints);
...
pub(crate) fn resolve_intrinsic_axis_constraints(
    ...
    ContentBasedSizeConstraints {
        ...
        preferred_aspect_ratio_applied: preferred.applied_aspect_ratio,
        min: min.value,
        max: max.value,
        automatic_min: None,
...
pub(crate) fn resolve_node_size_constraints(
    ...
    let auto_size = resolve_auto_size_preference(AutoSizeInput {
    ...
    resolved.size = auto_size.size;
    resolved.aspect_ratio_applied.width |= auto_size.aspect_ratio_applied.width;
    resolved.aspect_ratio_applied.height |= auto_size.aspect_ratio_applied.height;
    ...
    let own_definite_size = resolve_auto_size_preference(AutoSizeInput {
