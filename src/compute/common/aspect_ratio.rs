...
use crate::{ResolvedAspectRatio, Size};
...
pub(crate) struct ResolvedSizeConstraints {
    ...
    /// Used minimum constraints after merging authored and transferred values.
    pub min_size: Size<Option<f32>>,
    /// Used maximum constraints after merging authored and transferred values.
...
pub(crate) fn resolve_size_constraints(
    ...
    ResolvedSizeConstraints {
        size: size.maybe_apply_resolved_aspect_ratio(aspect_ratio, padding_border),
        min_size,
        max_size,
    }
}

/// Transfers each constraint into an auto-sized opposite axis through the
...
/// Merges a transferred minimum while allowing the authored maximum to cap it.
...
/// Merges a transferred maximum, then applies the CSS rule that minimum wins.
...
mod tests {
    ...
    fn ratio(value: f32) -> Option<ResolvedAspectRatio> {
        ResolvedAspectRatio::new(value, BoxSizing::BorderBox)
    }
