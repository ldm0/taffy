...
use super::common::absolute::{compute_absolute_layout, AbsoluteConstraintSpace};
...
pub fn compute_block_layout(
    ...
    if run_mode == RunMode::ComputeSize {
        ...
        // The inline contribution does not require resolving the block flow
        // when its own dimension is already known.
        if inputs.axis == RequestedAxis::from(writing_mode.inline_axis()) {
            if let Some(inline_size) = writing_mode.to_logical(styled_based_known_dimensions).inline_size {
...
fn compute_inner(
    ...
    let mut child_percentage_size = LogicalSize {
    ...
    if forward_parent_block_size {
        child_percentage_size.block_size = writing_mode.to_logical(parent_size).block_size;
    }
    ...
    let mut items =
        generate_item_list(tree, node_id, writing_direction, child_percentage_size, available_logical_space);

    // The intrinsic inline size is independent of the physical axis it occupies.
    ...
    // Forwarded percentages can change an anonymous wrapper's content block size
    // even without an aspect ratio. Keep those measurements constraint-keyed.
    let depends_on_block_constraints = content_inline_size_depends_on_block_constraints || forward_parent_block_size;

    // Short-circuit only for the requested physical axis whose logical size is known.
    if let (RunMode::ComputeSize, Some(container_outer_block_size)) = (run_mode, known_logical_size.block_size) {
        return LayoutOutput::from_outer_size(writing_mode.to_physical(LogicalSize {
    ...
    let container_percentage_resolution_block_size = if forward_parent_block_size {
        writing_mode.to_logical(parent_size).block_size
    } else {
        known_logical_size
            .block_size
            .or(size_logical.block_size.maybe_max(min_size_logical.block_size))
            .or(min_size_logical.block_size)
            .maybe_sub(logical_content_box_inset.block_axis_sum())
    };
    // Relative block-axis percentage insets only resolve against a definite
    // block size. A minimum can affect used size without making it definite.
    let relative_inset_percentage_resolution_block_size = if forward_parent_block_size {
        writing_mode.to_logical(parent_size).block_size
    } else {
        definite_logical_size
            .block_size
            .or(size_logical.block_size)
            .maybe_sub(logical_content_box_inset.block_axis_sum())
    };

    // Lay out the block flow before materializing any physical child positions.
            ...
            #[cfg(feature = "content_size")]
            {
                ...
                for item in items.iter() {
                    if let Some(pending) = item.pending_layout.as_ref() {
                        // Floats do not move with the alignment subject, but
                        // their unchanged fragments still contribute overflow.
    ...
    output.block_start_margin = if own_margins_collapse_with_children.start {
        first_child_block_start_margin_set
    } else {
        let margin =
    ...
    output.block_end_margin = if own_block_end_margin_collapses_with_children {
        last_child_block_end_margin_set
    } else {
    ...
    //
    // Parent block containers need the collapsing struts even during
    // size-only passes.
    if run_mode == RunMode::ComputeSize {
        return output;
    }
...
struct BlockContainerLayoutContext {
    ...
    /// Content-box logical block size available for descendant percentages.
    percentage_resolution_block_size: Option<f32>,
    /// Definite content-box logical block size available for relative percentage insets.
...
fn perform_final_layout_on_in_flow_children(
    ...
    let relative_inset_parent_size = writing_mode.to_physical(LogicalSize {
        inline_size: Some(container_inner_inline_size),
        block_size: relative_inset_percentage_resolution_block_size,
    });
    #[cfg(feature = "float_layout")]
    ...
    for item in items.iter_mut() {
        if item.position == Position::Absolute {
            ...
            // Tables and replaced elements own their inline sizing. An
            // orthogonal child's block axis must not acquire the containing
            // block's inline-axis stretch constraint either.
            let is_orthogonal = tree.get_writing_mode(item.node_id).is_orthogonal_to(writing_mode);
            let known_dimensions = if item.is_table || item.is_replaced || is_orthogonal {
            ...
            // Only a child in this BFC can export a collapsing margin strut.
            // A writing-mode root's struts use its own block direction, which
            // can be orthogonal or reversed relative to this container.
            let (child_start_margin, child_end_margin) = if item.is_in_same_bfc {
                (item_layout.block_start_margin, item_layout.block_end_margin)
            } else {
                (CollapsibleMarginSet::ZERO, CollapsibleMarginSet::ZERO)
            };
            let block_start_margin_set =
                child_start_margin.collapse_with_margin(item_margin.block_start.unwrap_or(0.0));
            let block_end_margin_set = child_end_margin.collapse_with_margin(item_margin.block_end.unwrap_or(0.0));

            // Expand auto margins to fill available space
            // Block-axis auto margins of in-flow block items resolve to zero.
            ...
            let mut logical_location = if item.is_in_same_bfc {
                ...
                #[cfg(feature = "float_layout")]
                let extra_margin_start = if item_avoids_floats {
                    resolved_logical_margin.inline_start - item_non_auto_margin.inline_start
                } else {
                    resolved_logical_margin.inline_start
                };
                #[cfg(not(feature = "float_layout"))]
                let extra_margin_start = resolved_logical_margin.inline_start;
...
fn perform_absolute_layout_on_absolute_children(
    ...
    for item in items.iter().filter(|item| item.position == Position::Absolute) {
        ...
        let static_available_space = Size {
            width: if writing_mode.is_axis_flow_reversed(AbsoluteAxis::Horizontal, direction) {
                static_edge.x - area_offset.x
            } else {
                area_size.width - (static_edge.x - area_offset.x)
            },
            height: if writing_mode.is_axis_flow_reversed(AbsoluteAxis::Vertical, direction) {
                static_edge.y - area_offset.y
            } else {
                area_size.height - (static_edge.y - area_offset.y)
            },
        };
        let computed = compute_absolute_layout(
            tree,
            item.node_id,
            AbsoluteConstraintSpace { containing_block_size: area_size, writing_direction, static_available_space },
        );
