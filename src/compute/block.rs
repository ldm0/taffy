                ...
                for item in items.iter() {
                    if let Some(layout) = item.final_layout.as_ref() {
                        let contribution_location = if direction.is_rtl() {
                            Point {
                                x: container_outer_width - (layout.location.x + layout.size.width) - border.right,
                                y: layout.location.y - border.top,
                            }
                        } else {
                            Point { x: layout.location.x - border.left, y: layout.location.y - border.top }
                        };
        ...
        if item.position == Position::Absolute {
            ...
            if let Some(float_direction) = item.float.float_direction() {
                ...
                #[cfg(feature = "content_size")]
                {
                    ...
                    let contribution_location = if direction.is_rtl() {
                        Point {
                            x: container_outer_width - (location.x + item_layout.size.width) - border.right,
            ...
            #[cfg(feature = "content_size")]
            {
                let contribution_location = if direction.is_rtl() {
                    Point {
                        x: container_outer_width - (location.x + final_size.width) - border.right,
                        y: location.y - border.top,
                    }
                } else {
                    Point { x: location.x - border.left, y: location.y - border.top }
