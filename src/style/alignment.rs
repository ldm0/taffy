...
//! modifier ([`AlignmentSafety`]). The pre-existing CSS spellings — `Normal`, `Start`,
//! `End`, `FlexStart`, `FlexEnd`, `Center`, `Stretch`, `SpaceBetween`, …, `SafeStart`,
...
pub enum AlignItemsKeyword {
    /// Use the formatting context's definition of the `normal` self-alignment value.
    Normal,
...
impl AlignItems {
    ...
    /// Resolve the context-dependent `normal` keyword without changing an
    /// authored non-normal value.
    #[inline]
